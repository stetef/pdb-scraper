#!/usr/bin/env python3
"""PDB download orchestration for the CLI.

``fetch_pdb`` itself now lives in :mod:`scrape_pdb.fetch` (stdlib ``urllib``
only) and is re-exported here, so the library path (``scrape_pdb.api``) never
imports this module and therefore never pulls in ``tqdm`` / ``requests``
(card P1.9, 04 §5, D-10). Re-exporting keeps ``scrape_pdb.downloader.fetch_pdb``
a module global, so existing monkeypatching in tests and callers is unchanged.
"""
