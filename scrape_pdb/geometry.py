...
def _planarity_metrics(points: list[tuple[float,float,float]]) -> tuple[float,float]:
    """Return RMS height from best plane and max height."""
    if len(points) < 3:
        return (0.0, 0.0)
...
def classify_geometry(center: Atom, neighbors: list[Atom]) -> tuple[str, dict[str, float], dict[str, str]]:
    ...
    sigma_d = float(np.std(dists)) if len(dists) > 1 else 0.0
    ...
    RMS_theta = float(np.sqrt(np.mean([(a-109.47)**2 for a in angles]))) if angles else 0.0  # baseline vs tetra
    ...
    # Heuristic flags
    planar_flag = "Yes" if RMS_plane <= 0.35 else "No"
    axial_flag = "Yes" if angles and max(angles) >= 160.0 else "No"
